...
bool parseParen( char array[], int n) {
    ...
    if( n == 0 ) {
       ...
       throw new Exception("Invalid state, parseParent expects n>=0");
    }
       ...
       switch(currentChar) {
		...
		openingParensToMatch.push(currentChar);
          ...
          case ')':
                ...
                break;
	  default: 
                throw new invalid_argument("Unexpected character " + currentChar  + found at position " + (currentPos+1) + "."); 
       }
       ++currentPos;
    } while ( i < n );

    bool allParensResolved = openingParensToResolve.size()==0
